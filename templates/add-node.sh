...
vRETURN=$(for i in $HOST_IPS ;do ssh -o StrictHostKeyChecking=no ${PX_ANSIBLE_USER}@${i} 'printf $(hostname)';printf ",${i} " ; done | xargs)
...
##Starting main process.
  ...
  vNODES=$(for i in $HOST_IPS ;do ssh -o StrictHostKeyChecking=no ${PX_ANSIBLE_USER}@${i} 'printf $(hostname)';printf "," ; done | xargs|sed 's/,$//g')
  ...
  if [[ "${PX_METALLB_ENABLED}" == "true" ]]; then
    ...
    ansible-playbook -i "${CONFIG_FILE}" "scale.yml" -u"${PX_ANSIBLE_USER}" -b --extra-vars "kubeconfig_localhost=true kubectl_localhost=true" --limit="${vNODES}"
  fi 
  cd ..
